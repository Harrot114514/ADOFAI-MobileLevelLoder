...
### 修改
- **页面重组**：设置页的游戏相关部分（不败模式、返回主页面）整合到独立
  「▣ 游戏」页；页签变为三个：文件选择 / 游戏 / 设置。
- **返回主页面按钮**：去掉醒目红色，改用主题默认按钮样式（非危险操作）。
- **日志清理**：设置页新增「清理日志」按钮，删除
  files/log/adofailoader.log（下一条日志自动重建）。

### 问题修复
- **清理日志后游戏卡死**：log_clear 持锁期间调用 LOGI（log_line 再次锁同一
  mutex）导致死锁 → 改为先释放锁再写日志。
- **部分中文显示 ??**：两处修复：① 字体魔数校验的 TTC 偏移读取错误
  （读了 numFonts 字段，正确偏移在 hdr[12..15]）→ 误拒正常字体 → 已修正；
  ② **重新生成内嵌字体子集**——v1.1.5+ 新增的 UI 字符串（游戏/清理日志/
  返回主页面等）此前未进子集，设备无可用系统字体时显示 ?；
  同时扩充系统字体候选路径（OnePlus/ColorOS 的 OPPOSans、HarmonyOS、
  DroidSansFallbackFull 等）。
- **游戏数据目录不再硬编码**：包名从 /proc/self/cmdline 动态获取
  （进程名 = 包名），默认目录/日志路径/设置文件路径全部运行时解析；
  游戏换包名后工具无需改动即可工作。

### 版本
- 工具内版本号同步至 **v1.1.7**（JNI_OnLoad 日志 + 设置页）。
