...
#define SAFE_HOOK(orig, rva, hk) do { \
    (orig) = (decltype(orig))hook_install((void*)(g_il2cpp_base + (rva)), (void*)(hk)); \
    if (!(orig)) LOGE("hook: %s FAILED", #orig); \
} while (0)

    SAFE_HOOK(orig_concat2,            RVA_string_Concat2,            &hk_concat2_c);
    SAFE_HOOK(orig_split,              RVA_string_Split,              &hk_split_c);
    SAFE_HOOK(orig_concat3,            RVA_string_Concat3,            &hk_concat3_c);
    SAFE_HOOK(orig_concat4,            RVA_string_Concat4,            &hk_concat4_c);
    SAFE_HOOK(orig_loadtexture,        RVA_TextureManager_LoadTexture,   &hk_LoadTexture_c);
    SAFE_HOOK(orig_loadsprite,         RVA_TextureManager_LoadNewSprite, &hk_LoadNewSprite_c);
    SAFE_HOOK(orig_rd_exists,          RVA_RDFile_Exists,             &hk_RDFile_Exists_c);
    SAFE_HOOK(orig_rd_readall,         RVA_RDFile_ReadAllBytes,       &hk_RDFile_ReadAllBytes_c);
    SAFE_HOOK(orig_difmode,            RVA_DetermineDifficultyUIMode, &hk_DetermineDifficultyUIMode_c);
    patch_write_u32((void*)(g_il2cpp_base + RVA_LoadTexture_exists_fail), 0xD503201F);

    SAFE_HOOK(orig_loadandplay,        RVA_scnGame_LoadAndPlayLevel,  &hk_LoadAndPlayLevel_c);
    SAFE_HOOK(orig_scngame_loadlevel,  RVA_scnGame_LoadLevel,         &hk_LoadLevel_c);
    SAFE_HOOK(orig_leveldata_loadlevel,RVA_LevelData_LoadLevel,       &hk_LevelData_LoadLevel_c);
    SAFE_HOOK(orig_get_isinternal,     RVA_ADOBase_get_isInternalLevel, &hk_get_isInternalLevel_c);

    SAFE_HOOK(orig_get_isdlclevel,     RVA_ADOBase_get_isDLCLevel,    &hk_get_isDLCLevel_c);
    SAFE_HOOK(orig_get_isbosslevel,    RVA_ADOBase_get_isBossLevel,   &hk_get_isBossLevel_c);
    SAFE_HOOK(orig_get_isclsbosslevel, RVA_ADOBase_get_isCLSBossLevel,&hk_get_isCLSBossLevel_c);

    SAFE_HOOK(orig_reloadsongco_movenext, RVA_ReloadSongCo_MoveNext,  &hk_ReloadSongCo_MoveNext_c);

    SAFE_HOOK(orig_reloadsong,         RVA_scnGame_ReloadSong,        &hk_ReloadSong_c);
    SAFE_HOOK(orig_reloadcustomsounds, RVA_scnGame_ReloadCustomSounds,&hk_ReloadCustomSounds_c);
